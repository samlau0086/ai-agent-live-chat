...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const conversation = await store.setConversationStatus(id, "closed", user.id);
  await store.addMessage({
    conversationId: id,
    role: "system",
    content: `${user.username} closed the conversation.`,
  });
  const updated = await store.getConversation(id);
  if (updated) publishConversation(updated);
  await emitWebhook("conversation.closed", conversation);
